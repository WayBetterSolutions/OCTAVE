...
def slugify(string: str) -> str:
    ...
    # Strip CJK characters (pykakasi removed — no Japanese romanization)
...
def format_query(
    ...
    formats = {
        ...
        "{album-artist}": song.album_artist or "",
        "{genre}": song.genres[0] if song.genres else "",
        "{disc-number}": song.disc_number if song.disc_number is not None else "",
        "{disc-count}": song.disc_count if song.disc_count is not None else "",
        "{duration}": song.duration if song.duration is not None else "",
        "{year}": song.year if song.year is not None else "",
        "{original-date}": song.date or "",
        "{track-number}": f"{int(song.track_number):02d}" if song.track_number else "",
        "{tracks-count}": song.tracks_count if song.tracks_count is not None else "",
        "{isrc}": song.isrc or "",
        "{track-id}": song.song_id or "",
        "{publisher}": song.publisher or "",
        "{output-ext}": file_extension,
        "{list-name}": song.list_name or "",
        "{list-position}": str(song.list_position).zfill(len(str(song.list_length))) if song.list_position is not None and song.list_length is not None else "",
        "{list-length}": song.list_length if song.list_length is not None else "",
    }
