import QtQuick 2.15
import QtQuick.Controls 2.15
import ".." as App

RadioButton {
    id: control

    indicator: Rectangle {
        implicitWidth: App.Spacing.dp(20)
        implicitHeight: App.Spacing.dp(20)
        x: control.leftPadding
        y: control.height / 2 - height / 2
        radius: App.EnvironmentTheme.active.radioSquare ? App.Spacing.dpMin(3, 1) : width / 2
        border.color: control.checked ? App.Style.accent : App.Style.secondaryTextColor
        border.width: 2

        Rectangle {
            width: App.Spacing.dp(10)
            height: App.Spacing.dp(10)
            anchors.centerIn: parent
            radius: App.EnvironmentTheme.active.radioSquare ? App.Spacing.dpMin(2, 1) : width / 2
            color: control.checked ? App.Style.accent : "transparent"

            Behavior on color { ColorAnimation { duration: 150 } }
        }
    }

    contentItem: Text {
        text: control.text
        color: App.Style.primaryTextColor
        verticalAlignment: Text.AlignVCenter
        leftPadding: control.indicator.width + control.spacing
        elide: Text.ElideRight
        width: control.width - control.indicator.width - control.spacing - App.Spacing.dp(10)
        font.family: App.Style.fontFamily
    }
}
